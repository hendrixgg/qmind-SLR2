...
# Rescale .jpg image to grayscale numpy array (28, 28, 1) with values 0 to 1
def rescale_image(img_path: str):
    return np.asarray(cv2.resize(cv2.cvtColor(cv2.imread(img_path), cv2.COLOR_BGR2GRAY), dsize=(28, 28), interpolation=cv2.INTER_CUBIC))
...
def main():
    ...
    ax3.set_axis_off()
    ax4.imshow(cv2.cvtColor(cv2.imread("LetterData/c/c0.png"), cv2.COLOR_BGR2GRAY), cmap='gray')
    ax4.set_axis_off()
    print(rescale_image("LetterData/c/c0.png").shape)
    ax5.imshow(rescale_image("LetterData/c/c0.png"), cmap='gray')
    ax5.set_axis_off()
    plt.show()
